...
$interfaceOutputPath = $unitOfWorkInterfaceName
$classOutputPath = $unitOfWorkClassName

# Add UoW Interface
Add-ProjectItemViaTemplate $interfaceOutputPath -Template IDefaultUnitOfWork `
	-Model @{ Namespace = $modelsNamespace; InterfaceName = $unitOfWorkInterfaceName; DomainClasses=$domainClasses; } `
	-SuccessMessage "Added UoW Interface at {0}" `
	-TemplateFolders $TemplateFolders -Project $DomainProjectName -CodeLanguage $CodeLanguage -Force:$Force

# Add UoW Implementation
Add-ProjectItemViaTemplate $classOutputPath -Template DefaultUnitOfWork `
	-Model @{ Namespace = $modelsNamespace; InterfaceName = $unitOfWorkInterfaceName; ClassName = $unitOfWorkClassName; EFConnectionName = $efConnectionName;  DomainClasses=$domainClasses; } `
